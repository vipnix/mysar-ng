...
switch ($a_switch) {
	...
	case 'IPSitesSummary' :
		...
		$query = 'SELECT ' . 'authuser' . ' FROM ' . 'users' . ' WHERE ' . 'id=\'' . $_REQUEST ['usersID'] . '\'';
		$currentAuthuser = db_select_one_row ( $link, $query );
		
		$query = 'SELECT ' . 'DISTINCTROW ' . 'users.id AS id' . ' FROM ' . 'users' . ' JOIN ' . 'trafficSummaries' . ' ON ' . 'users.id=trafficSummaries.usersID' . ' AND ' . 'users.date=trafficSummaries.date' . ' WHERE ' . 'users.authuser=\'' . $currentAuthuser ['authuser'] . '\'' . ' AND ' . 'users.date=\'' . $pageVars ['previousDate'] . '\'' . ' AND ' . 'trafficSummaries.ip=\'' . $pageVars ['hostiplong'] . '\'';
		$result = db_select_one_row ( $link, $query );
		$pageVars ['previousDateID'] = $result ['id'];
		
		$query = 'SELECT ' . 'DISTINCTROW ' . 'users.id AS id' . ' FROM ' . 'users' . ' JOIN ' . 'trafficSummaries' . ' ON ' . 'users.id=trafficSummaries.usersID' . ' AND ' . 'users.date=trafficSummaries.date' . ' WHERE ' . 'users.authuser=\'' . $currentAuthuser ['authuser'] . '\'' . ' AND ' . 'users.date=\'' . $pageVars ['previousWeek'] . '\'' . ' AND ' . 'trafficSummaries.ip=\'' . $pageVars ['hostiplong'] . '\'';
		$result = db_select_one_row ( $link, $query );
		$pageVars ['previousWeekID'] = $result ['id'];
		echo $pageVars ['previousWeekID'];
		
		$query = 'SELECT ' . 'DISTINCTROW ' . 'users.id AS id' . ' FROM ' . 'users' . ' JOIN ' . 'trafficSummaries' . ' ON ' . 'users.id=trafficSummaries.usersID' . ' AND ' . 'users.date=trafficSummaries.date' . ' WHERE ' . 'users.authuser=\'' . $currentAuthuser ['authuser'] . '\'' . ' AND ' . 'users.date=\'' . $pageVars ['nextDate'] . '\'' . ' AND ' . 'trafficSummaries.ip=\'' . $pageVars ['hostiplong'] . '\'';
		$result = db_select_one_row ( $link, $query );
		$pageVars ['nextDateID'] = $result ['id'];
		
		$query = 'SELECT ' . 'DISTINCTROW ' . 'users.id AS id' . ' FROM ' . 'users' . ' JOIN ' . 'trafficSummaries' . ' ON ' . 'users.id=trafficSummaries.usersID' . ' AND ' . 'users.date=trafficSummaries.date' . ' WHERE ' . 'users.authuser=\'' . $currentAuthuser ['authuser'] . '\'' . ' AND ' . 'users.date=\'' . $pageVars ['nextWeek'] . '\'' . ' AND ' . 'trafficSummaries.ip=\'' . $pageVars ['hostiplong'] . '\'';
		$result = db_select_one_row ( $link, $query );
