...
function getActiveUsers($link) {
    ...
    $query = "SELECT COUNT(DISTINCTROW ip) AS users FROM traffic WHERE date='" . $pageVars['date'] . "' AND time>'$time'";
    $recordSet = db_select_one_row($link, $query);
...
function url_createSortParameters($url, $validParameters) {
    ...
    foreach ($validParameters as $key => $value) {
        $tmpUrl = addParameter($url, 'OrderBy', $value);
        $newUrl[$value . 'ASC'] = addParameter($tmpUrl, 'OrderMethod', 'ASC');
        $newUrl[$value . 'DESC'] = addParameter($tmpUrl, 'OrderMethod', 'DESC');
    }
    return $newUrl;
}
