...
while ( ! feof ( $handle ) ) {
	...
	$query = "SELECT id FROM sites WHERE date='" . $dbRecord ['date'] . "' AND site='" . encrypt_decrypt ( 'encrypt', $dbUrl, $iniConfig ['key'] ) . "'";
	$recordSet = db_select_one_row ( $link, $query );
	if ($recordSet ['id'] == '') {
		debug ( 'Not found. Inserting to database...', 40, __FILE__, __LINE__ );
		$query = "INSERT INTO sites(site,date) VALUES ('" . encrypt_decrypt ( 'encrypt', $dbUrl, $iniConfig ['key'] ) . "','" . $dbRecord ['date'] . "')";
