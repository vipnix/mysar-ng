<nobr>[
<a href="{$smarty.server.PHP_SELF}?a=allsites&date={$pageVars.date}">&lt;&lt;&lt; Статистика за день</a>
|
...
<table>
  ...
  <form method="POST">
  <tr><td>Сайт</td><td style="text-align:left;"><a href="{$pageVars.site}" target="_blank">{$pageVars.site}</a></td></tr>
  </form>
</table>
    ...
    <tr>
      ...
      <th>
        <a href="{$smarty.server.PHP_SELF}?{$pageVars.url.bytesASC}"><img border="{$pageVars.bytesASCImageBorder}" src="images/up-arrow.gif"></a>
          {$pageVars.bytesLabelStart}Обьем{$pageVars.bytesLabelEnd}
